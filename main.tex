...
\citealp{delay_distrs} for a nice review). 

Given $\pi$, we can express the
...
&= \sum_{k=0}^d X_{t-k} \, \P(\text{secondary after $k$ time steps} \given 
  \text{secondary occurs}) \,
\P(\text{secondary occurs} \given\text{primary at $t-k$}) \nonumber \\ 
\label{eq:model}
...
primary events at $t$ produce secondary events after $t$, their effects are not
...
This paper is focused on severity rates and not reproduction numbers, however,
the results we develop in studying the former may also be of use for
understanding the latter, given the connections between these frameworks. We
briefly return to this point in the discussion.   
...
\end{equation}
where \smash{$A_t^\gamma$} compares how the delay distributions convolve against
the most recent primary incidence levels,
\[
A_t^\gamma = \frac{\sum_{j=0}^d X_{t-j}\pi_j}{\sum_{j=0}^d X_{t-j} \gamma_j}.
\]
\end{proposition}
...
the convolutional \eqref{eq:conv} and lagged ratio \eqref{eq:lagged} estimators 
...
explore three different underlying HFR curves and two delays. In addition to
...
feedback. This work was supported by Centers for Disease Control and Prevention
