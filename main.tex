...
% The ratios in Equations \eqref{eq:lagged} and \eqref{eq:conv} track a time-varying severity rate. To estimate the average rate over all time, they are often used with cumulative counts. That is, each time series is the sum of all counts from the first timestep. This version of \eqref{eq:conv} is unbiased if the severity rate is stationary, and is widely used in practice \citep{nishiura}. For the time-varying setting, however, the lagged ratio is much more common.
...
% As defined in \eqref{eq:severity}, the severity rate is analogous to the case reproduction number - the average number of secondary infections produced by a single infection at time $t$. 
% For reproduction numbers, the primary and secondary events are infections; for severity rates, the secondary event is a different type of outcome, which can be generated at most once by a primary event. 
...
% Secondary events produced by primary events at present time $t$ are not observed in real time. 
% Real-time estimates target instantaneous $R_t$, the average number of secondary infections at time $t$ produced by a single primary infection in the past. This is analogous to the severity ratio estimates \eqref{eq:lagged} and \eqref{eq:conv}, which relate current secondary events to past primary events. 
...
% time-varying severity ratios are biased when the true rates are changing.
% Assume the true delay distribution is a constant $\pi$ over all time with maximum length $d$. We first analyze the convolutional ratio \eqref{eq:conv}, assuming oracle knowledge of the true delay distribution $\pi$.
...
Theorem \ref{thm:misp}, proven in Appendix~\ref{apx:proofs}, provides an additive decomposition of the misspecified ratio's bias. In both terms, $A_t^\gamma$ dictates the extent to which the misspecified distribution alters the bias. The first term scales the oracle bias, whereas the other solely expresses misspecification. Which of these two terms will dominate depends on the true severity rate $p_t$, the oracle bias, and the ratio $A_t^\gamma$. If oracle bias is small, for example, then its multiplicative scaling should have relatively little effect, in which case the misspecification term may drive bias. In the experiments that follow, the second term seems to contribute more bias. TO DO: Elaborate.
% The extent of the change from the oracle bias depends on the ratio $A_t^\gamma$. 
...
% When the plug-in delay distribution hits less mass of the incidence curve - for example, if $\gamma$ is lighter-tailed than $\pi$ and primary events are falling - then $A_t^\gamma > 1$. 
...
% While the well-specified analysis assumes the true delay distribution is known, different choices of delay distribution generally yield the same bias (Appendix~\ref{apx:robustness}). 
% The convolutional estimator in green may not be far from the oracle ratio, though the lagged ratio has misspecification bias introduced in Section~\ref{sec:misp}. 
...
Next, the misspecified analysis explains central discrepancies between the convolutional and lagged ratios. The lagged ratio's erratic behavior during Omicron is almost identical to the simulated results in Figure \ref{fig:misspecified}. We attributed the bias to changes in hospitalization counts, which affect $A_t^\ell$ in Theorem \ref{thm:misp}.

% The lagged estimates dip below the ground truth and convolutional HFRs as hospitalizations fall around February 2022. Shortly thereafter, they spike even higher than the convolutional ratio. These biases both occured in Figure \ref{fig:misspecified}, where deaths were simulated from the same hospitalizations and HFRs. Our analysis attributed this bias to changing hospitalization counts, affecting $A_t^\ell$ in Theorem \ref{thm:misp}.

In addition, the lagged ratio is briefly less negatively biased in August 2021, during the Delta wave. Like the spike in January 2022, this can be explained by the sharp rise in hospitalizations. The surge causes $A_t^\ell$ to rise above 1, so the additive term in \ref{thm:misp} is positive. This briefly offsets the negative oracle bias, bringing the lagged ratio closer to the true HFR. Similarly, the lagged bias is less \textit{positive} in February 2021. Here, $A_t^\ell < 1$ due to falling hospitalizations, so the additive term is negative. 
% Our analysis attributed the bias to $A_t^\ell$ in Theorem \ref{thm:misp}, affected by changes in hospitalization counts.
...
% Section~\ref{sec:ws_analysis} demonstrated that the oracle convolutional ratio is unbiased when the severity rate is a constant $p$. Theorem \ref{thm:misp} simplifies to $\mathbb{E}[\hat{p}_t^\gamma] - p = p(A_t^\gamma - 1)$; rearranging and recalling $\mathbb{E}[\hat{p}_t^\gamma] = \hat{p}_t^\gamma$ when deaths are simulated without noise, $A_t^\gamma = \frac{\mathbb{E}[\hat{p}_t^\gamma]}{p}$.
...
% To mimic the real data, we first used the same HFRs from NHCS used for validation in Section~\ref{sec:results_real}. We also inverted them and rescaled in order to simulate the opposite trend. Lastly, we explored a stationary HFR of 10\% over all time. 
...
% mirroring Figures \ref{fig:bias_ex_main}, \ref{fig:misspecified}, and \ref{fig:basic_est_vs_gt_figs}. 
...
Figure \ref{fig:sims} displays the results on the six settings of delay distribution and HFR. In addition to the HFR curves, it also visualizes the ratios $A_t^\gamma$, which merely rescale the HFR estimates in the stationary case. Given a constant rate $p$, the oracle convolutional ratio is unbiased, so Theorem \ref{thm:misp} reduces to $\mathbb{E}[\hat{p}_t^\gamma] = p A_t^\gamma$. Furthermore, $\mathbb{E}[\hat{p}_t^\gamma] = \hat{p}_t^\gamma$, as our setup simulates deaths without noise. Consequently, $A_t^\gamma = \frac{\hat{p}_t^\gamma }{p}$. 
...
Our analysis accounts for the wide gap in performance between the two estimators. For example, equation Theorem \ref{thm:OracleBias} guarantees the convolutional ratio is unbiased under stationarity. In contrast, Theorem \ref{thm:misp} highlights the lagged estimator's misspecification error when hospitalizations are changing. The analysis here is the same as in Section~\ref{sec:misp}. During the Delta and Omicron waves, rapid rises in hospitalizations produced high values of $A_t^\ell$. This accounts for the spikes in August 2021 and January 2022, across all 3 HFR settings. 
...
Next, we prove the bias expression under misspecified delay distribution (Theorem \ref{thm:misp}). 
\begin{proof}
    \begin{align*}
...
% Had the following in, but removed because it's not particularly insightful. 
