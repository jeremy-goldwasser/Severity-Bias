...
\textbf{Example.} Suppose half of the secondary events occur after exactly $a$ days, and the other half after $b>a$. Further assume severity rate is the same $a$ days before $t$, but different $b$ days prior. Then%Define $c\geq 0$ such that $X_{t-a} = cX_{t-b}$, indicating the proportion change in primary incidence between $t-a$ and $t-b$. Then 
% $$\text{Bias}(\hat{p}_t^{\text{Conv}}) =  \frac{X_{t-b}(p_{t-k}-p_t)}{X_{t-b}+X_{t-a}} = \frac{p_{t-k}-p_t}{1+c}$$
\begin{align*}
...
HFRs obtained from aggregate hospitalization and death counts are significantly higher than those from NHCS because not all deaths occur in hospitals. A CDC analysis reported the percentage of inpatient deaths every month from 2020 through 2022; roughly 60\% of Covid deaths occurred in hospitals in 2022, down from nearly 70\% in 2021 and 2022. To account for non-inpatient deaths, we divided the NHCS curve by these percentages after again smoothing with a spline. 
