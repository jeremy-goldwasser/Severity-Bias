...
severity rates \djmcomment{``wrong severity rates'' doesn't sound quite right.
...
necessarily be too high or low.''}. This statistical bias arises as a
consequence of changing severity rates --- precisely when time-varying estimates
should be most useful. Bias is influenced by changes in the primary incidence
curve (e.g. cases, in CFR), as well as long time delays between events. 
...
though the true HFRs fell. We study the sources of this bias, and suggest
alternative methodology which overcomes it.
...
of lag and delay distribution. \djmcomment{I try to avoid referring to future
...
(before 3.1). It's a good summary of what comes next there.} Appendix~\ref{apx:robustness} evaluates the
...
the two time series. The delay distribution is defined as \djmcomment{I found
...
\end{equation*}
%
Several tools exist to estimate delay distributions from aggregate or
line-list data \citep{delay_distrs}. For ease of analysis, we consider discrete
...
$p$, \eqref{eq:model} simplifies to $E[Y_t] = p\sum_{k=0}^d X_{t-k}\pi_k$.
\citet{nishiura} rearranged this expression to estimate this time-stationary rate
using a plug-in estimate of the delay distribution and smoothing over the entire
history,
...
defined here.} The convolutional ratio \eqref{eq:conv} can be understood as a
generalization of \eqref{eq:lagged}. It reduces to the same ratio that
${\gamma}$ is a point mass distribution where all secondary events occur after
$\ell$ days. 
...
$\pi$ is the renewal equation $g$, measuring the time between primary and
secondary infections. \djmcomment{My understanding is that the renewal equation
is actually the whole thing: $y_t = R_t\sum_{k=1}^\infty g_{k} y_{t-k}$. Maybe you
mean ``generation interval distribution $g$''?}
...
\begin{theorem}\label{thm:OracleBias}
    Assume the true delay distribution is a known constant $\pi$ over all time
    with maximum length $d$ \djmcomment{What does maximum length mean here?}. The bias of the convolutional ratio
    $\hat{p}_t^\pi$ is
\begin{equation*}
...
so that the sum is a bit more clear. Feel free to revert.} Appendix \ref{apx:proofs} contains the short proof.
\end{theorem}
...
\begin{figure}
     ...
     \end{subfigure}
        ...
        factors. Deaths were computed noiselessly from \eqref{eq:model}, with
...
next few paragraphs. Like ``Figure 2 illustrates\ldots''.} Suppose primary events have stabilized after falling for a long time (see
April 2022 in Fig.~\ref{fig:misspecified}). If the plug-in delay distribution is
...
of $\pi$ --- and none in the long tail of $\pi$. In the flattened-out period
...
above, which emphasizes recent high counts. Figure~\ref{fig:misspecified}
...
\begin{figure}
    ...
    \caption{HFR estimates under misspecification. HHS hospitalizations, NCHS HFRs, and noiseless deaths from Eq.~\eqref{eq:model}. Convolutional ratio estimates with true delay distribution (mean 20), misshapen gammas (mean 16 and 24), and point mass at correlation-maximizing lag (16).}%(i.e. lag) at oracle mean
...
estimators \djmcomment{as described in Theorems 1 and 2 when applied to\ldots}. Both the lagged and convolutional ratios respond very slowly to
...
\ref{fig:basic_est_vs_gt_figs}, and \ref{fig:sims}). While Equation
\eqref{eq:nish} from \citet{nishiura} is widely used to compute stationary or
...
focuses on. Section~\ref{sec:setup} mentioned, for example, the fact that
estimating HFR from aggregates fails to address the large proportion of deaths
that occur outside the hospital; \citet{lipsitch2015potential} refers to this as
``survivorship bias." A central challenge for CFR estimation is under-reporting:
Not all events are reported, reporting rates change across time, and severe
cases are more likely to be reported than mild cases.
\citet{reich2012estimating} proposes an estimator for a time-invariant
...
distribution affects its results. They also identify other sources of bias, like
