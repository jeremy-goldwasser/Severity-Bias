...
% Assume the delay distribution is a constant $\pi$ for all time
...
% The above ratios are designed to estimate a time-varying severity rate. To estimate the average rate over all history, Equations \ref{eq:lagged} and \ref{eq:conv} are often used with cumulative counts. 
...
\begin{figure}
     ...
     \begin{subfigure}[b]{0.45\linewidth}
         ...
         \caption{Constant primary incidence. All deaths after $\ell$ days. HFR ratios equivalent; plotting delays of $\ell=14$ and 28 days.}
         \label{fig:onehot}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{0.45\linewidth}
...
We further evaluated these methods on simulated deaths whose true HFRs is known. Throughout these experiments, we used observed, finalized hospitalization counts from HHS as the primary incidence curve $X_t$.  For a series of time-varying HFRs $p_t$ and delay distribution $\pi$, deaths are defined as
$$Y_t := \sum_{k=0}^d X_{t-k} \mathbb{P}(\text{die at $t$ }\vert\text{ hosp at }t-k) = \sum_{k=0}^d X_{t-k} \pi_k p_{t-k}.$$
