...
case and death counts were widely used to estimate and report COVID-19 CFRs, both
in the academic literature \citep{yuan2020monitoring, timevar_ifr,
...
We now analyze the bias of the convolutional ratio \eqref{eq:conv} in what we
call the \emph{well-specified} case, when the true delay distribution $\pi$ is
...
Assume that the true delay distribution $\pi$ is known and fixed over all time,  
and all secondary events must occur within $d$ time steps. Then the bias of the 
convolutional ratio \smash{$\hat{p}_t^\pi$} is 
\begin{equation}
\label{eq:OracleBias}
...
Appendix \ref{apx:proofs}. The proposition reveals that the bias of the
convolutional ratio estimator of the severity rate depends on three factors,
which we discuss next. Figure \ref{fig:bias_ex_main} provides an accompanying
illustration.  

\begin{enumerate}
\item \textbf{Changes in severity rate}. The central component of this bias
  expression is the difference $p_{t-k}-p_t$. When the true severity rate is
  constant over the $d$ preceding days, the convolutional ratio is unbiased
  (because this difference is zero). This is in line with the motivation used to
  derive this estimator, as explained in the last subsection. But when
  ...
  falling, with $p_t < p_{t-1} < \ldots < p_{t-d}$. The bias will then be
...
\item \textbf{The primary incidence curve.} Changing primary incidence $X_t$ 
  ...
  terms $X_{t-k}\pi_k(p_{t-k} - p_t)$ for dates further from the present, which 
...
\caption{Simple examples which illustrate the effects of the three factors
  ...
  curve, COVID-19 deaths, from \eqref{eq:model} without noise. The true HFR 
  curve $p_t$ and delay distribution $\pi$ used in the simulation were ground
  truth proxies derived from external data sources, as explained in more detail
...
\begin{proposition}\label{prop:MispBias}
...
For the lagged estimator, \eqref{prop:MispBias} becomes
...
Proposition \ref{prop:MispBias}, proven in Appendix~\ref{apx:proofs}, provides an
...
% This occurs even when the rescaled oracle and misspecification terms are at odds with one another. For example, $A_t^\gamma<1$ pulls negative oracle bias upwards towards 0, while adding negative misspecification error. 
...
% This occurs even when the oracle bias and misspecification terms have different signs. 
% Whenever the oracle bias is positive, for example, 
% If oracle bias is relatively small, then its multiplicative scaling should have
...
% The plug-in distribution $\gamma_k=\mathds{1}\{k=\ell\}$ does not always mirror the same smooth delay distribution. 
The plug-in distribution $\gamma_k=\mathds{1}\{k=\ell\}$ convolves all its mass at $X_{t-\ell}$. 
During the steepest phase of the rise, this is considerably lower than $X_{t}$. %$X_{t-\ell}$
...
% Throughout the entire rise, $A_t^\gamma$ is above or below 1 for smooth $\gamma$. 
...
In some settings, it may even attain the worst-case delay distribution in Proposition~\ref{prop:MispBias}. 
...
We first prove Proposition \ref{prop:OracleBias}, the bias of the oracle convolutional ratio. 
\begin{proof}
...
Next, we prove the bias expression under misspecified delay distribution (Proposition \ref{prop:MispBias}). 
\begin{proof}
    \begin{align*}
