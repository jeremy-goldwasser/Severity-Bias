...
The above section considered the bias of the convolutional ratio where the true delay distribution $\pi$ is known. We now consider the more general case, in which it is instead replaced with a plug-in estimate $\gamma$. Nevertheless, we still assume $\pi$ is constant over time, and its maximal length $d$ exceeds that of the plug-in distribution. Note the bias of the lagged estimator is a special case, where the plug-in distribution is a point mass at lag time $\ell$. 
...
\end{align}
% This can be further decomposed into the oracle error \ref{eq:ConvBias}. Letting $\hat\Delta_k = \gamma_k - \pi_k$,
...
% If we assume $\sum_{j=0}^d X_{t-j}\pi_j = \sum_{j=0}^d X_{t-j}\gamma_j$, then 

% \begin{align}\label{eq:ConvBiasMisp}
%     \text{Bias}(\hat{p}_t^\gamma) = \text{Bias}(\hat{p}_t^\pi) + p_t\sum_{k=0}^d \frac{X_{t-k}}{\sum_{j=0}^d X_{t-j}\gamma_j}(\gamma_k -\pi_k).
% \end{align}
...
% % \begin{align}\label{eq:LagBias}
% %     \text{Bias}(\hat{p}_t^\ell) &= \frac{E[Y_t]}{X_{t-L}} - p_t \nonumber\\ 
...
% separate $\gamma$ from oracle $\pi$, and then
% specializing it to the cases where we know
% the oracle, so $\gamma=\pi$, and where
% $\gamma$ puts all mass on $\ell$?
...
% % How do estimates of $\gamma$ affect the bias
