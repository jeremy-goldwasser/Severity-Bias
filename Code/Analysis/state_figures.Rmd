...
jhu_deaths <- readRDS( file.path(data_folder, "JHU_finalized.RData"))
...
# Load NHCS HFRs
nhcs_hfrs_all <- readRDS(file.path(data_folder, "HFRs_NHCS_rescaled.RData"))
...
## Estimate true HFRs, and smooth approx. ground truth
...
### Repeat on properly-versioned data
...
rtPlot <- ggplot(data=dfHFR, aes(x = Date, y = HFR, color = Method, linetype=Method)) + 
  geom_line() + labs(color = "HFR", linetype = "HFR") + 
  ggtitle("US HFRs, Ratio Estimates vs Approximate Ground Truth", subtitle="Real-time counts") +
  scale_x_date(breaks = seq(first_xaxis_date, max(dfHFR$Date), by = "3 months"), date_labels = "%b %Y") +
  xlab("") + theme_bw() +
...
Plot <- ggplot(data=df, aes(x = Date, y = HFR, color = type, linetype=source)) +
  geom_line() + labs(color = "HFR", linetype = "HFR") +
  ggtitle("US HFRs, Ratio Estimates vs Approximate Ground Truth") +#, subtitle="Real-time counts"
  scale_x_date(breaks = seq(as.Date("2020-10-01"), max(df$Date), by = "3 months"), date_labels = "%b %Y") +
  xlab("") + theme_bw() +
  theme(legend.position="bottom",
        legend.title=element_blank())
Plot
...
for (i in 1:length(waves)) {
  ...
  hosps_period <- hhs_hosps_realtime[names(hhs_hosps_realtime) %in% bad_dates]
  gt_hfrs_period <- hfrsNHCS_rt[est_weeks_realtime %in% bad_dates]
  deaths_period <- jhu_deaths_realtime[names(jhu_deaths_realtime) %in% bad_dates]
