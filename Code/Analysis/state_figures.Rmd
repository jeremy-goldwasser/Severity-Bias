...
for (i in 1:6) {
  ...
  dfHFR <- dfList[[i]]
  lags <- oracle_lags[[i]]
  dfHFR = dfHFR |> mutate(Method = recode_factor(Method, "Approx. GT" = "External estimate"))# Approximate GT
...
StatePlot <- (plotlist[[1]] + plotlist[[2]]) /
  ...
  plot_annotation(title="Ratio and external estimates",
...
ggsave(file.path(figure_folder, real_dir, "state_level_hfrs.pdf"), 
