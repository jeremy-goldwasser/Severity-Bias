...
n_days_after <- 6
week_st_idx <- 5
est_weeks_realtime <- est_weeks[week_st_idx:n_ests] #5
# 
...
#                                    geo_type = "nation", geo_values="*", 
#                                    time_type="day", as_of=t+6, time_values=dates))
#   new_hosps <- hhs_df$value; names(new_hosps) <- as.Date(hhs_df$time_value)
#   
#   # Account for bad delay distr
#   # new_hosps <- ifelse(i==1, new_hosps[names(new_hosps) >= first_hosp_date], new_hosps)
#   if (i==1) {
...
# length(hhs_hosps_realtime)==max(as.Date(names(hhs_hosps_realtime)))-min(as.Date(names(hhs_hosps_realtime)))+1
...
# length(jhu_deaths_realtime)
# max(as.Date(names(jhu_deaths_realtime)))-min(as.Date(names(jhu_deaths_realtime)))+1

# sort(table(names(jhu_deaths_realtime)), decreasing=TRUE)[1:10]
# jhu_deaths_realtime[names(jhu_deaths_realtime)=="2020-12-14"]
...
# oracle_lag_nchs <- calc_optimal_lag(hhs_hosps, nchs_deaths, verbose=TRUE)
hfrsHNlretro <- compute_lagged_hfrs(hhs_hosps, nchs_deaths,
                              l=oracle_lag_nchs, w=7, dates=est_weeks_nchs, real_time=FALSE, centered=FALSE)
