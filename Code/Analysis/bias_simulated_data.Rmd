...
HFRtypes <- c("GT", "Lagged", "Conv")
dfHFRshort <- data.frame(Date=rep(est_dates, length(HFRtypes)),
                 HFR=c(gt_hfrs, hfrs_lagged_short, hfrs_conv_short), 
...
dfHFRlong <- data.frame(Date=rep(est_dates, length(HFRtypes)),
                 HFR=c(gt_hfrs, hfrs_lagged_long, hfrs_conv_long), 
                 Method=factor(rep(HFRtypes, each=length(est_dates)), levels=HFRtypes))
...
dfHFRflat <- data.frame(Date=rep(est_dates, length(HFRtypes)),
                 HFR=c(gt_hfrs_flat, hfrs_lagged_short_flat, hfrs_conv_short_flat), 
                 Method=factor(rep(HFRtypes, each=length(est_dates)), levels=HFRtypes))
...
gt_hfrs_flat <- flat_hfrs[hosp_dates %in% est_dates]
dfHFRflat <- data.frame(Date=rep(est_dates, length(HFRtypes)),
                 HFR=c(gt_hfrs_flat, hfrs_lagged_long_flat, hfrs_conv_long_flat), 
                 Method=factor(rep(HFRtypes, each=length(est_dates)), levels=HFRtypes))
ggplot(data=dfHFRflat, aes(x = Date, y = HFR, color = Method, linetype=Method)) + 
  geom_line() + labs(color = "HFR", linetype = "HFR") + 
  ggtitle("Simulation: Estimated vs True HFRs (Flat)") +
...
dfHFRshort_inv <- data.frame(Date=rep(est_dates, length(HFRtypes)),
                 HFR=c(gt_hfrs_inv, hfrs_lagged_short_inv, hfrs_conv_short_inv), 
                 Method=factor(rep(HFRtypes, each=length(est_dates)), levels=HFRtypes))
...
dfHFRlong_inv <- data.frame(Date=rep(est_dates, length(HFRtypes)),
                 HFR=c(gt_hfrs_inv, hfrs_lagged_long_inv, hfrs_conv_long_inv), 
                 Method=factor(rep(HFRtypes, each=length(est_dates)), levels=HFRtypes))
...
p1 <- ggplot(data=dfHFRshort, aes(x = Date, y = HFR, color = Method, linetype=Method)) + 
  ...
  ggtitle("NHCS HFRs",
          subtitle="Short Delay Distribution") +
  xlab("") + theme_bw() +
  theme(legend.position="none") #plot.title = element_text(size=titlesize)
...
p3 <- ggplot(data=dfHFRshort_inv, aes(x = Date, y = HFR, color = Method, linetype=Method)) + 
  geom_line() + labs(color = "HFR", linetype = "HFR") + 
  ggtitle("Inverted NHCS HFRs") +#subtitle="Short Delay Distribution"
  xlab("") + theme_bw() +
  theme(legend.position="none")
...
HFRtypes2 <- c("Ground Truth", "Lagged Ratio", "Convolutional Ratio")
dfHFRflat <- data.frame(Date=rep(est_dates, length(HFRtypes2)),
                 HFR=c(gt_hfrs_flat, hfrs_lagged_short_flat, hfrs_conv_short_flat), 
                 Method=factor(rep(HFRtypes2, each=length(est_dates)), levels=HFRtypes2))
...
dfHFRflat2 <- data.frame(Date=rep(est_dates, length(HFRtypes2)),
                 HFR=c(gt_hfrs_flat, hfrs_lagged_long_flat, hfrs_conv_long_flat), 
                 Method=factor(rep(HFRtypes2, each=length(est_dates)), levels=HFRtypes2))
