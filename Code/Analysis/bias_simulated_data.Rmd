...
hhs_hosps <- readRDS(file.path(data_folder, "HHS.RData"))
...
```{r}
lag_short <- calc_optimal_lag(hhs_hosps, sim_deaths_short)
...
lag_long <- calc_optimal_lag(hhs_hosps, sim_deaths_long)
...
lag_short <- calc_optimal_lag(hhs_hosps, sim_deaths_short_flat)
...
lag_long <- calc_optimal_lag(hhs_hosps, sim_deaths_long_flat)
...
lag_short <- calc_optimal_lag(hhs_hosps, sim_deaths_short_inv)
...
lag_long <- calc_optimal_lag(hhs_hosps, sim_deaths_long_inv)
...
```{r}
plugin_delay_distr <- rep(0, d+1); plugin_delay_distr[]
compute_R_gammas(hosps, delay_distr_short, plugin_delay_distr, est_dates)
```
