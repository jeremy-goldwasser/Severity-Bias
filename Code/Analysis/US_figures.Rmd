...
```{r}
approx_name <- "External estimate"

df_real_time <- df_real_time |>
  mutate(
    Method = case_match(Method, "Approx. GT" ~ approx_name, .default = Method),
    Method = factor(Method,
                    levels = c(approx_name,
                               "Convolutional ratio",
                               "Lagged ratio"))
  )
# df_real_time = df_real_time |>
#   mutate(Method = case_match(Method, "Approx. GT" ~ approx_name, .default = Method))
...
rtPlot <- ggplot(data=df_real_time, aes(x = Date, y = HFR, color = Method, linetype=Method)) + 
  ...
  ggtitle("Ratio and external estimates")+
...
rtPlot
ggsave(file.path(figure_folder, real_dir, "US_ests_realtime.pdf"),
...
df_both <- df_both |>
  mutate(
    ...
    source = factor(source, 
                    levels = c("GT", "Finalized", "Real-time"))
  )
...
bothPlot <- ggplot(data=df_both, aes(x = Date, y = HFR, color = type, linetype=source)) +
  ...
  ggtitle("Ratio and external estimates", subtitle="Using both real-time and finalized counts") +
...
bothPlot
ggsave(file.path(figure_folder, real_dir, "US_ests_realtime_both.pdf"),
...
for (i in 1:length(waves)) {
  ...
  dfAgg$Legend_GT_HFR <- approx_name
 ...
 # ymax <- max(lagged_hfrs_period, gt_hfrs_period, conv_hfrs_period, hosps_period*scale)*1.1
  ...
  p <- ggplot(dfAgg, aes(x = Date)) +
  ggtitle(wave) +
  geom_line(aes(y = Conv_HFRs, color = Legend_Conv_HFR, linetype = Legend_Conv_HFR)) +
  geom_line(aes(y = Lagged_HFRs, color = Legend_Lagged_HFR, linetype = Legend_Lagged_HFR)) +
  geom_line(aes(y = GT_HFRs, color = Legend_GT_HFR, linetype = Legend_GT_HFR)) +
  geom_line(aes(y = (Hosps - a) * scale + b, color = Legend_HHS, linetype = Legend_HHS)) +
  scale_linetype_manual(
    breaks = c("External estimate", "Convolutional ratio", "Lagged ratio", "Hospitalizations"),
    values = c("External estimate" = "solid",
               "Convolutional ratio" = "dotted",
               "Lagged ratio" = "dashed",
               "Hospitalizations" = "dotdash")
  ) +
  scale_color_manual(
    breaks = c("External estimate", "Convolutional ratio", "Lagged ratio", "Hospitalizations"),
    values = c("External estimate" = "red",
...
wavePlot
ggsave(file.path(figure_folder, real_dir, "hfrs_by_wave.pdf"),
...
df_window_conv = df_window_conv |>
  mutate(Window = recode_factor(Window, "Approx. GT" = approx_name))
...
ggsave(file.path(figure_folder, real_dir, "window_size_conv.pdf"), 
       plot = p1, device = "pdf", width = 10, height = 4) 

ggsave(file.path(figure_folder, real_dir, "window_size_lagg.pdf"), 
...
```{r}
df_lags = df_lags |> 
  mutate(Lag = recode_factor(Lag, "Approx. GT" = approx_name))
...
lagPlot
ggsave(file.path(figure_folder, real_dir, "hfrs_by_lag.pdf"),
...
df_delays_9 = df_delays_9 |> 
  mutate(Method = recode_factor(Method, "GT" = approx_name))
...
DelayPlot
ggsave(file.path(figure_folder, real_dir, name), plot = DelayPlot, 
...
DelayPlot2
ggsave(file.path(figure_folder, real_dir, name), plot = DelayPlot2, 
       device = "pdf", width = 10, height = 4) 
```
...
deathSourcePlot <- ggplot(df_deaths_by_source, aes(x=Date, y=Deaths, color=Source, linetype=Source)) + 
  ...
  theme(legend.position="bottom", 
        ...
        plot.title = element_text(size = 17))
deathSourcePlot
ggsave(file.path(figure_folder, real_dir, "death_curves.pdf"), 
...
altGT <- ggplot(data=df_hfr_types, aes(x = Date, y = HFR, color = Method, linetype = Method)) + 
  ...
  ggtitle("External estimate comparison") + 
  theme_bw() + xlab("") +
  theme(legend.position="bottom",
        plot.title=element_text(size=17),
        legend.text=element_text(size=14),
...
altGT
ggsave(file.path(figure_folder, real_dir, "ApproxGT.pdf"), 
...
HFRsourcePlot
ggsave(file.path(figure_folder, real_dir, "jhu_vs_nchs.pdf"), 
