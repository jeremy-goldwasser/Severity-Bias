...
ggplot(df, aes(x=t, y=HFRs, color=Type, linetype=Source)) +
...
ggplot(df, aes(x = t, y = HFRs, linetype = Source, color = Type)) +
  geom_line(aes(group = interaction(Source, Type))) +  # Group by Source and Type
  scale_color_manual(values = c("Ground Truth" = "black", "Short (12)" = "red2", "Med (20)" = "green3", "Long (28)" = "blue"),
                     breaks = types) +  # Manually set color, exclude Ground Truth from legend
  ggtitle("Delay Distribution") +
  labs(color="Delay Length") + xlab("Date")
```
...
```{r}
# rev_hosps <- (hosps_window-min(hosps_window))*0.1+mean(hosps_window)
rev_hosps <- rev(hosps_window); names(rev_hosps) <- names(hosps_window)

# plot(hosps_window, type="l"); lines(rev_hosps)
hfrs_true_hosps <- calc_nish_hfrs(hosps_window, chging_hfrs, delay_distr_med)
hfrs_rev_hosps <- calc_nish_hfrs(rev_hosps, chging_hfrs, delay_distr_med)

hfr_range <- max(chging_hfrs)-min(chging_hfrs)
shifted_og_hosps <- (hosps_window-min(hosps_window))/max(hosps_window)*hfr_range
shifted_og_hosps <- (shifted_og_hosps-min(shifted_og_hosps))+min(chging_hfrs)
shifted_flat_hosps <- (rev_hosps-min(rev_hosps))/max(rev_hosps)*hfr_range
shifted_flat_hosps <- (shifted_flat_hosps-min(shifted_flat_hosps))+min(chging_hfrs)
# shift <- mean(chging_hfrs)/mean(hosps_window)

sources <- c("GT HFRs", "Estimated", "Hospitalization")
sources_df <- factor(c(rep(sources[1], n_ests), rep(sources[2], n_ests*2), rep(sources[3], n_ests*2)), levels=sources)
# types <- c("True Hosps", "Flatter Hosps")
types <- c("GT HFRs", "True Hosps", "Reversed Hosps")
types_df <- factor(c(rep(types[1], n_ests), rep(rep(types[2:3], each=n_ests),2)), levels=types)

df <- data.frame(t=t0+rep(1:n_ests, 5)-1,
                 data=c(chging_hfrs[(d+1):n_pts], hfrs_true_hosps, hfrs_rev_hosps, 
                        shifted_og_hosps[(d+1):n_pts], shifted_flat_hosps[(d+1):n_pts]),
                 Source=sources_df,
                 Type=types_df)

ggplot(df, aes(x=t, y=data, color=Type, linetype=Source)) +
