...
DATA_PATH = os.path.join(THIS_DIR, "../../Data/Real_data/Variants")
...
# print(len(not_states)) # Should be 7

# with open(SEQCOUNTS_CSV_PATH) as f:
#     perCountryData = json.load(f)["regions"][1]["distributions"]
with open(SEQCOUNTS_CSV_PATH) as f:
    perCountryData = [country for country in json.load(f)["regions"][1]["distributions"] if country['country'] == "USA"]
...
# seq_df_X = seq_df.set_index('Date').groupby('State').resample('1D').mean().reset_index() #['Alpha', 'Beta', 'Epsilon', 'Iota', 'Gamma', 'Delta', 'Omicron', 'Other'].ffill().reset_index()
# print(seq_df_X.shape)
seq_df.to_csv(os.path.join(DATA_PATH, "seq_df_us_biweekly.csv"), index=False)
...
# seq_df_filled.to_csv(os.path.join(DATA_PATH, "seq_df_us.csv"))
