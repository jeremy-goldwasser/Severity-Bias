...
## Get HFRs as (smoothed NHCS HFRs)/(smoothed monthly counts)

```{r}
spline_model <- smooth.spline(as.numeric(months), percentages, spar=0.6)
pct_deaths_in_hospital <- predict(spline_model, x = as.numeric(in_hosp_days))$y

# Plot original data and the smoothed curve
plot(months, percentages, main = "COVID-19 Hospital Inpatient Deaths (%)", ylim=c(0,100),
     xlab = "Months Since January 2020", ylab = "Percentage", type = "b", pch = 19)
lines(in_hosp_days, pct_deaths_in_hospital, col = "red")

nhcs_hfrs_all <- readRDS(file.path(data_folder, "smoothed_hfrs.RData"))
nhcs_hfr_dates <- as.Date(names(nhcs_hfrs_all)) 
lag <- 0# 25 # Reasonable hosp-to-death delay. Not so important
rescaled_hfr_dates <- as.Date(intersect(nhcs_hfr_dates, (in_hosp_days-lag)))

frac_deaths_in_hospital_sub <- pct_deaths_in_hospital[in_hosp_days %in% (rescaled_hfr_dates+lag)]/100
nhcs_hfrs_sub <- nhcs_hfrs_all[nhcs_hfr_dates %in% rescaled_hfr_dates]

nhcs_hfrs_rescaled_all <- nhcs_hfrs_sub/frac_deaths_in_hospital_sub
# plot(nhcs_hfrs_rescaled_all, type="l")

saveRDS(nhcs_hfrs_rescaled_all, file.path(data_folder, "hfrs_rescaled_v1.RData"))
```
...
lag <- 0# 25 # Reasonable hosp-to-death delay. Not so important
rescaled_hfr_dates <- as.Date(intersect(daily_hfr_dates, (in_hosp_days-lag)))
...
plot(rescaled_hfr_dates, smoothed_hfrs, type="l")
lines(as.Date(names(nhcs_hfrs_rescaled_all)), nhcs_hfrs_rescaled_all, type="l", col="red")
legend("topright", legend = c("Smooth Raw/Raw", "Smooth/Smooth"), col = c("black", "red"), lty = 1)
```
...
nhcs_hfrs_sub <- nhcs_hfrs_all[nhcs_hfr_dates %in% est_days]

nhcs_hfrs_rescaled <- nhcs_hfrs_rescaled_all[names(nhcs_hfrs_rescaled_all) %in% est_days]
trans <- mean(jhu_deaths_est)/mean(hhs_hosps_est)/mean(nhcs_hfrs_rescaled)
...
plot(est_days, nhcs_hfrs_rescaled, ylim=c(0,.3), type="l", main="Rescaled HFRs (naive and data-driven)")
lines(est_days, nhcs_hfrs_rescaled2, col="red")

exp_deaths_est <- sapply(est_weeks, function(t) {
  trailing_dates <- seq(t-d, t, by="day")
  trailing_hfrs <- nhcs_hfrs_rescaled[est_days %in% trailing_dates]*trans
...
versions <- c("JHU", "Expected (ratio of smooth)", "Expected (smooth raw ratio)", "Expected (naive rescaling)")
deathsDF <- data.frame(Date=rep(est_weeks,4),
                       Deaths=c(jhu_deaths_est, exp_deaths_est, exp_deaths_est2, exp_deaths_est3),
